...
class DichiarazioneIntento(models.Model):
    ...
    taxes_ids = fields.Many2many('account.tax', string='Taxes',
                                 required=True)
    used_amount = fields.Float(compute='_compute_amounts', store=True)
    limit_amount = fields.Float(required=True)
    available_amount = fields.Float(compute='_compute_amounts', store=True)
    ...
    @api.multi
    @api.depends('limit_amount', 'line_ids', 'line_ids.amount')
    ...
    def get_valid(self, type=None, partner_id=False, date=False):
        ...
        records = self.search(domain, order='state desc, date asc')
        return records
